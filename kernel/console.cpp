...
void Console::Newline() {
    cursor_column_ = 0; // カーソル位置を左端にする。
    if (cursor_row_ < kRows - 1) { // 最終行でなければカーソル位置を一行下げる。
        ++cursor_row_;
        return;
    }
    ...
    } else { //最終行の場合は、背景で塗りつぶしてバッファデータをもとに一行ずつ上にずらしながら描画する。
        FillRectangle(*writer_, {0, 0}, {8 * kColumns, 16}, bg_color_);
...
void Console::Refresh(){
    for (int row = 0; row < kRows; ++row) {
        WriteString(*writer_, Vector2D<int>{9, 16 * row}, buffer_[row], fg_color_);
    }
}